...
class Telescope:
	...
	def printTelescope(self):
		...
		print("Latitude: %s\tLongitude: %s" %(self.Observer.location.latitude, 
				self.Observer.location.longitude))
		print("Current time is %s" %(self.getTime()))
		#print("Telescope park location: %s, %s" %(self.park.az, self.park.alt))
	...
	def slewToCoord(self, ra, dec, time=None):
		...
		if not time: time = self.getTime()
		coord = SkyCoord(ra, dec)
		if not self.Observer.target_is_up(time, coord):
			print("Target RA: %s, Dec: %s is below the horizon.")
			return
...
#		print("Finished parking")
	...
	def record(self, outFile=None):
		...
		'''
		if not outFile: outFile = str(datetime.today()) + '.txt'
